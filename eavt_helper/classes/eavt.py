...
class EAVT:
    def __init__(self, eavt_path: str):
        ...
        try:
            ...
            if missing_columns:
                raise ValueError(f"EAVT file missing required columns: {missing_columns}. "
            ...
            try:
                ...
                raise ValueError(f"Cannot convert time column 't' to datetime: {str(e)}")
    ...
    def transform_to_scd2(self, chunk_size: Optional[int] = None) -> pd.DataFrame:
        """
    ...
    def _process_in_chunks(self, chunk_size: int) -> pd.DataFrame:
        ...
        with click.progressbar(range(0, total_entities, entities_per_chunk), 
                             label='Processing entity chunks') as bar:
            for i in bar:
    ...
    def _transform_single_chunk(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return (df.pipe(self._with_pivot_eavt)
    ...
    def _with_pivot_eavt(self, df):
        ...
        try:
            ...
            raise ValueError(f"Error pivoting EAVT data: {str(e)}. "
