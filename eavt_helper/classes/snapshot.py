...
class Snapshot:
    def __init__(self, snapshot_path: str, id_col: str, tstamp_col: str):
        ...
        try:
            ...
            if id_col not in self.df.columns:
                raise ValueError(
                    f"ID column '{id_col}' not found in snapshot. Available columns: {list(self.df.columns)}"
                )
            if tstamp_col not in self.df.columns:
                raise ValueError(
                    f"Timestamp column '{tstamp_col}' not found in snapshot. Available columns: {list(self.df.columns)}"
                )
            ...
            try:
                ...
                raise ValueError(
                    f"Cannot convert timestamp column '{tstamp_col}' to datetime: {str(e)}"
                ) from e
            ...
            click.echo(
                f"✓ Loaded snapshot with {len(self.df)} rows and {len(self.df.columns)} columns"
            )
    ...
    def transform_to_eavt(self, chunk_size: int | None = None) -> pd.DataFrame:
        """
    ...
    def _transform_single_chunk(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return (
            df.pipe(self._with_set_index, [self.id_col, self.tstamp_col])
