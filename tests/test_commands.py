...
class TestSnapshotToEavtCommand:
    ...
    def test_snapshot_to_eavt_success(self):
        ...
        runner = CliRunner()
        result = runner.invoke(snapshot_to_eavt, [
            '--snapshot-path', self.input_file.name,
            '--id-column', 'user_id',
            '--tstamp-column', 'timestamp',
            '--out-path', self.output_file.name
        ])
...
class TestEavtToScdCommand:
    ...
    def test_eavt_to_scd_success(self):
        ...
        runner = CliRunner()
        result = runner.invoke(eavt_to_scd, [
            '--eavt-path', self.input_file.name,
            '--out-path', self.output_file.name
        ])
        ...
        expected_cols = ['id', 'row_effective_tstamp', 'row_expiration_tstamp', 
